...
const categories = ["Electronics","Smartphones","Computers & Laptops","Audio & Headphones","Watches","Fashion & Clothing","Shoes","Bags & Pouches","Toys & Games"];
const units = ["cm", "inches", "liters", "kg", "pieces"];
const colors = ["Red", "Blue", "Green", "Yellow", "Black", "White", "Gray", "Custom"];
const sizes = ["XS", "S", "M", "L", "XL", "XXL", "Custom"];
const statusColors: Record<string,"default"|"success"|"warning"|"error"|"info"> = { Published: "success", Draft: "default", "Low Stock": "error" };
...
export default function ProductsPage() {
  ...
  const [form, setForm] = useState<Partial<Product> & { images?: File[] }>({
    name: "",
    category: "",
    stock: 0,
    price: 0,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setOpen(false);
      setForm({ name:"", category:"", stock:0, price:0, unit:"", description:"", color:"", size:"", images: [] });
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleDelete = async (id: string) => { await dispatch(deleteProduct(id)).unwrap(); };
    ...
    <Box sx={{ p:2 }}>
          ...
          <TableBody>
            {products.map((p) => (
              <TableRow key={p._id}>
                ...
                <TableCell>
                  <Chip label={p.status || "Draft"} color={statusColors[p.status || "Draft"]} size="small" variant="outlined"/>
                </TableCell>
                <TableCell align="center">
                  <IconButton color="primary" size="small" onClick={()=>handleEdit(p)}><Edit fontSize="small"/></IconButton>
                  <IconButton color="error" size="small" onClick={()=>handleDelete(p._id!)}><Delete fontSize="small"/></IconButton>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
      ...
      <Dialog open={open} onClose={()=>setOpen(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent dividers>
          ...
          {form.color==="Custom" && <TextField fullWidth margin="dense" label="Custom Color" name="customColor" value={form.customColor} onChange={handleChange}/>}
          ...
          {form.size==="Custom" && <TextField fullWidth margin="dense" label="Custom Size" name="customSize" value={form.customSize} onChange={handleChange}/>}
